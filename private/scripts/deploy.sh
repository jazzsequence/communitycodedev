...
ran_wait=false
while true; do
  running_workflow=$(terminus workflow:list "$TERMINUS_SITE.dev" --format=json | jq -r '.[] | select(.finished_at == null and (.workflow | test("Multidev"; "i") | not)) | (.workflow | gsub("\""; ""))' 2> /dev/null)
