<?php
/**
 * Plugin Name: CC AI Connector Priority
 * Description: Admin settings page to configure which AI provider is tried first for each task type (text, image, vision). Priorities are saved to wp_options and applied via the WordPress AI plugin's preference filters.
 */

namespace CommunityCode\AiConnectorPriority;
...
add_action(
	...
	static function (): void {
		add_options_page(
			__( 'AI Connector Priority', 'community-code' ),
			__( 'AI Priority', 'community-code' ),
			'manage_options',
			PAGE_SLUG,
...
function render_page(): void {
	...
	$tasks      = [
		'text'   => [
			'label'       => __( 'Text Generation', 'community-code' ),
			'description' => __( 'Used for: title generation, excerpt, summarization, content resizing, editorial notes, meta descriptions.', 'community-code' ),
		],
		'image'  => [
			'label'       => __( 'Image Generation', 'community-code' ),
			'description' => __( 'Used for: featured image generation, inline image generation, image editing. Anthropic does not support image generation.', 'community-code' ),
		],
		'vision' => [
			'label'       => __( 'Vision', 'community-code' ),
			'description' => __( 'Used for: alt text generation, image analysis.', 'community-code' ),
		],
	];
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'AI Connector Priority', 'community-code' ); ?></h1>
		<p><?php esc_html_e( 'Set which provider to try first for each AI task type. Only providers with a configured API key in Settings → Connectors are used. If the first-choice provider is not registered, the next one is tried automatically.', 'community-code' ); ?></p>

		<?php if ( $saved ) : ?>
			<div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'Priority settings saved.', 'community-code' ); ?></p></div>
		<?php endif; ?>
		...
		<form method="post">
			...
			<?php foreach ( $tasks as $task => $info ) : ?>
				...
				<table class="form-table" role="presentation">
					...
					$labels        = [
						__( '1st choice', 'community-code' ),
						__( '2nd choice', 'community-code' ),
						__( '3rd choice', 'community-code' ),
					];
			...
			<?php submit_button( __( 'Save Priority Settings', 'community-code' ) ); ?>
		</form>
	</div>
	<?php
}
...
function save_priorities(): bool {
	...
	$raw = isset( $_POST['cc_ai_priority'] ) && is_array( $_POST['cc_ai_priority'] )
		? $_POST['cc_ai_priority']
		: [];
